...
export default function SessionSidebar() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-transparent text-fg-primary">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-border bg-bg-base/50 backdrop-blur-sm select-none">
        <label
          htmlFor="session-sort"
          className="text-xs font-bold uppercase tracking-widest text-fg-muted select-none shrink-0"
        >
          Sort By
        </label>
        <div className="relative flex-1">
          <select
            ...
            className="w-full bg-bg-elevated/40 hover:bg-bg-elevated/80 border border-border/80 text-fg-secondary hover:text-fg-primary px-2.5 py-1 text-xs font-medium outline-none cursor-pointer appearance-none pr-7 transition-all duration-200 focus:border-accent-dim/60 focus:ring-1 focus:ring-accent-dim/30 shadow-sm rounded-md"
          >
        ...
        {sessions.length > 0 ? (
          Array.from(groupedSessions.entries()).map(([group, items]) => (
            <div key={group} className="space-y-1.5">
              ...
              <div className="px-4 text-xs font-bold uppercase tracking-widest text-fg-muted select-none mb-2 flex items-center gap-2">
                <span>{group}</span>
              ...
              {items.map((session) => (
                ...
                >
                  {/* Indicator bar left */}
                  <div
                    className={`absolute left-0 top-1/2 -translate-y-1/2 w-0.75 rounded-r-full transition-all duration-200 ${
                      session.active
                  ...
                  <div className="flex-1 flex items-start gap-2.5 min-w-0 pl-1">
                    {/* Icon representation */}
                    <div
                      className={`mt-0.5 w-3.5 h-3.5 shrink-0 transition-colors ${
                        session.active
                          ? "text-accent"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-col items-start gap-1 mt-0.5">
                        ...
                        {session.workspace && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-bg-subtle border border-border text-fg-tertiary max-w-full truncate font-mono select-none leading-none">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center text-fg-muted px-5 py-12 select-none">
