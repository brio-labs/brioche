...
export default function MemoryPanel({ onClose }: MemoryPanelProps) {
	...
	return (
		<PanelOverlay title="Memory" onClose={onClose} panelClassName="bg-bg-surface border border-border rounded-lg w-[600px] max-w-[95vw] max-h-[85vh] flex flex-col overflow-hidden animate-slideUp shadow-2xl z-[1001]">
			<div className="flex flex-col h-full min-h-0 overflow-hidden p-5 gap-4">
				...
				{!isTauriAvailable && (
					<div className="bg-error-bg text-error-text border border-error-border px-3.5 py-2.5 rounded-lg text-xs mx-4 my-3 shrink-0">
						Memory panel preview mode: changes require the Tauri desktop app.
					</div>
				)}
				{error && <div className="bg-error-bg text-error-text border border-error-border px-3.5 py-2.5 rounded-lg text-xs mx-4 my-3 shrink-0">{error}</div>}

				<div className="flex-1 overflow-y-auto min-h-0 flex flex-col gap-3 py-1">
					{memories.length === 0 ? (
						<div className="text-center text-fg-muted py-12 px-4 text-sm select-none">No memories yet</div>
					) : (
						memories.map((memory) => (
							<div key={memory.key} className="p-3 bg-bg-elevated/30 border border-border rounded-lg flex flex-col gap-1.5 transition-all hover:border-border-hover">
								<div className="flex items-center justify-between gap-2">
									<span className="font-mono text-xs font-semibold text-fg-primary">{memory.key}</span>
									<div className="flex items-center gap-2">
										...
										<button
											className="p-1.5 text-fg-muted hover:text-error-text hover:bg-bg-highlight rounded transition-colors cursor-pointer flex items-center justify-center shrink-0"
								...
								<div className="text-sm text-fg-secondary whitespace-pre-wrap leading-relaxed px-0.5">{memory.value}</div>
								<div className="text-[10px] text-fg-dim px-0.5 select-none">
