...
export default function MemoryPanel({ onClose }: MemoryPanelProps) {
	...
	return (
		<PanelOverlay title="Memory" onClose={onClose} size="sm">
				...
				{!isTauriAvailable && (
					<div className="bg-error-bg text-error-text border border-error-border px-3.5 py-2.5 rounded-lg text-xs shrink-0">
						Memory panel preview mode: changes require the Tauri desktop app.
					</div>
				)}
				{error && <div className="bg-error-bg text-error-text border border-error-border px-3.5 py-2.5 rounded-lg text-xs shrink-0">{error}</div>}
							...
							<div key={memory.key} className="p-3 bg-bg-elevated/30 border border-border rounded-lg flex flex-col gap-1.5 transition-all hover:border-border-hover">
								<div className="flex items-center justify-between gap-2">
									...
									<div className="flex items-center gap-2">
										<span className="px-1.5 py-0.5 rounded text-xs font-medium bg-accent/10 border border-accent/20 text-accent font-sans uppercase tracking-wider select-none">{memory.category}</span>
										<button
								...
								<div className="text-xs text-fg-dim px-0.5 select-none">
