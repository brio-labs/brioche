...
function getFileIcon(ext: string) {
  switch (ext) {
    case "pdf":    return <FileText className="h-7 w-7 text-red-400 shrink-0" />;
...
function AttachmentCard({ content }: { content: string }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-[6px] border border-white/10 bg-white/5 px-3 py-2.5 mt-1 max-w-xs">
      {getFileIcon(ext)}
      <div className="flex flex-col min-w-0">
        <span className="truncate text-[12px] font-semibold text-white/90" title={fileName}>{fileName}</span>
        <span className="truncate text-[10px] text-white/40 font-mono" title={filePath}>{filePath}</span>
      </div>
    </div>
  );
}
    ...
    <div className="flex-1 overflow-y-auto flex flex-col relative">
      ...
      <div className="flex flex-col gap-0 px-0 py-6 max-w-3xl mx-auto w-full">
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg) => {
            /* ── Tool call / result ─────────────────────── */
            if (msg.role === "tool_request" || msg.role === "tool_result" ||
                msg.role === "tool_argument" || msg.role === "tool_done") {
              return (
                <motion.div
                  id={`msg-${msg.id}`}
                  key={msg.id}
            ...
            if (msg.role === "system" || msg.role === "error") {
              ...
              return (
                ...
                >
                  <div className={`
                    inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-[11px] font-mono
                    ${isError
                  ...
                  `}>
                    {isAttachment
                      ? <>📎 {msg.content.substring("Attached: ".length).trim().split(/[/\\]/).pop()}</>
                      : msg.content
                    }
                  </div>
                </motion.div>
                  ...
                  <div className="flex flex-col items-end gap-1 max-w-[75%] min-w-0">
                    ...
                    ">
                      ...
                      {attachParts.map((part, i) => {
                        if (part.startsWith("Attached: ")) return <AttachmentCard key={i} content={part} />;
                        if (part.match(/^!\[/)) return <ImageAttachment key={i} content={part} />;
                        return null;
                      })}
            ...
            return (
              ...
              >
                {/* Avatar */}
                <div className="mt-1 h-7 w-7 shrink-0 rounded-full bg-bg-elevated border border-border flex items-center justify-center text-accent">
