//! Benchmark: `stream_zero_alloc` — Sprint 18.
//!
//! Verifies zero heap allocations in the Pass/Hold stream hot path.
//! The benchmark itself measures the path; a separate `#[test]` in
//! `tests/property_transition.rs` uses a counting allocator to assert
//! zero allocations.
//!
...
fn bench_stream_zero_alloc(c: &mut Criterion) {
    ...
    c.bench_function("stream_zero_alloc", |b| {
        b.iter(|| {
...
criterion_group!(benches, bench_stream_zero_alloc);
